...
function nav(event: any) {
	...
	if (st > lastScrollTop) {
		(_a = document.getElementById("nav")) === null || _a === void 0
			? void 0
			: _a.classList.remove("top-0");
		_a.classList.add("top-[-6rem]");
	} else {
...
export default function Example() {
	...
	const navclick = (e: any) => {
		...
		parent.childNodes.forEach((node: any, index: any) => {
					...
					<div className="relative w-full">
						<Popover>
							{({ open }) => (
								<>
									<div
										id="nav"
										className="fixed bg-white dark:bg-gray-900 transition-all ease-in-out top-0 left-0 z-10 py-6  w-full"
									>
										<nav
											className="relative flex items-center px-4 sm:px-6 justify-between"
											aria-label="Global"
										>
											<div className="flex items-center flex-grow flex-shrink-0 lg:flex-grow-0">
												<div className="flex items-center justify-between w-full md:w-auto">
													<a href="#">
														...
														<svg
															width="500"
															className="h-10 w-10 opacity-0 animate-fade-in-down [animation-delay:100ms]"
															height="500"
															viewBox="0 0 500 500"
															fill="none"
													...
													<div className="-mr-2 flex items-center md:hidden">
														<Popover.Button className="bg-white dark:bg-gray-900 rounded-md p-2 inline-flex items-center justify-center text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500">
															...
															<MenuIcon
																className="h-8 w-8 opacity-0 animate-fade-in-down [animation-delay:150ms]"
																aria-hidden="true"
															/>
														</Popover.Button>
													</div>
											...
											<div className="hidden md:flex md:ml-10 md:pr-4 md:gap-x-8">
												{navigation.map((item, i) => (
													<a
														...
														className={`mono font-medium opacity-0 [animation-delay: ${
															150 + 125 * i
														}] animate-fade-in-down text-gray-500 dark:text-white hover:text-indigo-500 focus:text-indigo-500`}
													>
														:{item.name}
													</a>
												))}
												<a
													...
													className="opacity-0 animate-fade-in-down [animation-delay:520ms] mono font-medium text-indigo-500 focus:text-indigo-500"
												>
												...
												{mounted ? (
													<Switch
														...
														} relative opacity-0 animate-fade-in-down [animation-delay:630ms] inline-flex items-center h-6 rounded-full w-11`}
													>
									...
									<Popover.Overlay
										className={`${
											open ? "opacity-30 inset-0" : "opacity-0"
										} bg-black transition-all ease-in-out top-0 z-10 fixed`}
									/>
													...
													<div className="mr-2 my-auto">
														{mounted ? (
															<Switch
																...
																className={`${
																	enabled ? "bg-blue-600" : "bg-gray-200"
																} relative inline-flex items-center h-6 rounded-full w-11 my-auto mr-2`}
															>
																<span className="sr-only">
																	Enable notifications
																</span>
						...
						<main className="relative mx-auto max-w-7xl px-6 sm:px-6 lg:px-8 h-screen flex flex-col md:justify-center">
							<div className="text-left md:max-w-2xl mt-48 md:mt-0">
								<h5 className="mono mb-1 opacity-0 animate-fade-in-up [animation-delay:800ms] text-base text-lg text-gray-500 dark:text-gray-400 sm:max-w-xl md:mb-2 sm:mx-0 md:text-xl lg:mx-0">
									Hello, I am
								</h5>
								<h1 className="text-4xl text-base tracking-tight font-extrabold text-gray-900 dark:text-white sm:text-5xl md:text-6xl">
									<span className="block opacity-0 animate-fade-in-up [animation-delay:950ms] xl:inline leading-normal">
										Ashwin Kr. Sharma
									</span>{" "}
									<span className="block opacity-0 animate-fade-in-up [animation-delay:1150ms] text-indigo-500">
										I <span className="line-through">build</span> engineer
										things for the web
									</span>
								</h1>
								<p className="mt-3 opacity-0 animate-fade-in-up [animation-delay:1250ms] text-lg text-gray-500 sm:mt-5 sm:text-lg dark:text-gray-400 leading-normal sm:max-w-xl sm:mx-0 md:mt-5 md:text-xl lg:mx-0">
								...
								<div className="mt-16 sm:mt-8 sm:flex justify-start">
									<div className="rounded-md opacity-0 animate-fade-in-up [animation-delay:1300ms] shadow w-[200px]">
										<a
											href="#contact"
						...
						<footer className="flex flex-col align-center">
							<div className="flex gap-x-4 justify-center mb-4 lg:fixed lg:bottom-24 lg:right-24 lg:flex-col lg:gap-x-0 lg:gap-y-10">
								<a
									href="https://github.com/ashwincreates"
									target="_blank"
									rel="noopener noreferrer"
								>
									<GitHub className="animate-fade-in-up opacity-0 [animation-delay:1350ms] dark:text-white" />
								</a>
								<a
									...
									rel="noopener noreferrer"
								>
									<Linkedin className="dark:text-white animate-fade-in-up opacity-0 [animation-delay:1425ms]" />
								</a>
							</div>
